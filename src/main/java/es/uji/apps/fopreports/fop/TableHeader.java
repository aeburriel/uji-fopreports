 ...
 *       &lt;choice>
 *         &lt;element ref="{http://www.w3.org/1999/XSL/Format}table-row" maxOccurs="unbounded"/>
...
public class TableHeader {
    ...
    public TableRow withTableRow(int index) {
        List<TableRow> list = this.getTableRow();
        if (list.size()<= index) {
            for (int i = list.size(); (i<= index); i ++) {
                list.add(null);
            }
        }
        TableRow value = list.get(index);
        if (value == null) {
            value = new TableRow();
    ...
    public TableRow withNewTableRow() {
        TableRow value = new TableRow();
        this.getTableRow().add(value);
        return value;
    }

    public TableCell withTableCell(int index) {
        List<TableCell> list = this.getTableCell();
        if (list.size()<= index) {
            for (int i = list.size(); (i<= index); i ++) {
                list.add(null);
            }
        }
        TableCell value = list.get(index);
        if (value == null) {
            value = new TableCell();
            list.set(index, value);
        }
    ...
    public TableCell withNewTableCell() {
        TableCell value = new TableCell();
        this.getTableCell().add(value);
        return value;
    }
